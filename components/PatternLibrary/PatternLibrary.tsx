...
const PatternLibrary = () => {
  const colors = ['orange', 'cyan', 'pink', 'blue', 'white', 'black']
          ...
          {colors.map((color) => {
            return (
              <div className={styles.colorsPalette} key={color}>
                <h2 className={styles.colorsPaletteTitle}>{color}</h2>
                <div
                  className={cx(
                    styles.colorsPaletteColor,
                    styles[`colorsPaletteColor__${color}`]
                  )}
                />
              </div>
            )
